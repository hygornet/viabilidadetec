...
(function () {
  ...
  function aplicarVisualCampo(input, tipo) {
  if (!input) return;
  ...
  if (tipo === 'inteiro') {
    input.value = inteiroParaVisual(valorAtual);
    return;
  }

  if (tipo === 'decimal') {
    input.value = decimalVisual(valorAtual);
    return;
  }

  if (tipo === 'decimal_m2') {
    input.value = decimalVisual(valorAtual, 'm²');
    return;
  }
  ...
  function aplicarMascaras() {
    ...
    Object.keys(campos).forEach(sel => {
      ...
      pegarCampos(sel).each(function () {
        const $f = $(this);
        // Aplica a máscara também quando o campo já vem preenchido ao carregar
        aplicarVisualCampo(this, tipo);
  ...
  function boot() {
      ...
      // reforço para Power Pages / Step Form que carrega campos depois
      setTimeout(aplicarMascaras, 200);
      setTimeout(aplicarMascaras, 600);
      setTimeout(aplicarMascaras, 1200);
    }
...
$(document).ready(function () {
  ...
  const observerMunicipio = new MutationObserver(function () {
    const $gridModal = $(".modal-dialog .entity-grid").filter(function () {
      return isGridMunicipio($(this));
    });
  ...
  observerMunicipio.observe(document.body, {
    childList: true,
    subtree: true
  });
});
...
function aplicarRegraTipoCadastro() {
    ...
    // Só reseta para NÃO quando o usuário acabou de mudar de não-CNPJ para CNPJ
